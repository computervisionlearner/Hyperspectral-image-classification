...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  1 10:30:04 2018
...
def center_crop(images, per):
  
  return tf.map_fn(lambda x: tf.image.central_crop(x, per), images, dtype=tf.float32)
  


class HSI_branch:
  
  def __init__(self, height = 17, width =17, channel = 48, classes = 20, batch_size = 512):
  ...
  def __call__(self, images, keep_prob):
    ...
    return logits1, variables1, logits2, variables2, logits, outputs, variables
  
  
  def conv2d(self, images):
    
    rate = self.rate
    images = tf.random_crop(images, (self.batch_size,16,16,self.channel))
...
#    inputs = images[:,r-6:r+5+1,r-6:r+5+1,:]
    ...
    inputs = center_crop(images, np.random.uniform(0.47,0.7))
    ...
    out2 = tf.image.resize_images(out2, (6,6))
